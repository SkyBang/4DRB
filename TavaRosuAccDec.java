...
@Autonomous(name= "TavaRosuAcDec")
public class TavaRosuAccDec extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        telemetry.update();
        //am actualizat datele din telemetrie
        ...
        Laterala1Dreapta(0.3,450);

        telemetry.addData("encoder value","enc");
        ...
        //CurbaFataStanga(0.8,0.5,3550);
        CurbaFataStanga(0.8,0.5,3250);
        while (opModeIsActive() && !isStopRequested() && MotorSF.isBusy() && MotorSJ.isBusy() && MotorDF.isBusy() && MotorDJ.isBusy()) {
            angles = imu.getAngularOrientation(AxesReference.INTRINSIC, AxesOrder.ZYX, AngleUnit.DEGREES);
            telemetry.addData("Heading: ", angles.firstAngle);
            telemetry.addData("Roll: ", angles.secondAngle);
    ...
    public void Laterala1Dreapta(double p,int t){
        MotorSF.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        MotorSJ.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        MotorDF.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        MotorDJ.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
        ...
        if (MotorSF.getCurrentPosition() <100 ){
            p = 0.2;
            MotorSF.setPower(p);
            MotorSJ.setPower(p);
            MotorDF.setPower(p);
        ...
        else if ((MotorSF.getCurrentPosition() >=100) &&(MotorSF.getCurrentPosition() <= 440)){
            p = 0.4;
            MotorSF.setPower(p);
            MotorSJ.setPower(p);
            MotorDF.setPower(p);
    ...
    public void Catretava1(double p,int t){
        ...
        if (MotorSF.getCurrentPosition() <200 ){
            p = 0.2;
            MotorSF.setPower(p);
            MotorSJ.setPower(p);
            MotorDF.setPower(p);
        ...
        else if ((MotorSF.getCurrentPosition() >=200) &&(MotorSF.getCurrentPosition() <= 1900)){
            p = 0.6;
            MotorSF.setPower(p);
            MotorSJ.setPower(p);
            MotorDF.setPower(p);
